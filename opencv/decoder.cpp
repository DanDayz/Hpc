...
int main(){
    ...
    uchar *image_data;
    image = imread("outputs/test1.png",CV_LOAD_IMAGE_COLOR);   // Read the file, BGR format
    Size s = image.size();
    int width = s.width;
    int height= s.height;
    ...
    while (!file.eof()){
      ...
      string binary_char = bitset<8>((int)my_character).to_string();
        ...
        for(int k = 0 ;k < 3 ; k++){ // D: ugly form to solution the general problem, maybe i will fix in a future.
