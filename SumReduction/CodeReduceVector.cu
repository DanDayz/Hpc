...
__global__ void KernelNormalVec(double *g_idata,double 
*g_odata,int l){

         __shared__ double sdata[TILE_WIDTH];
    // each thread loads one element from global to shared mem
    unsigned int tid = threadIdx.x;
    unsigned int i = blockIdx.x*blockDim.x + threadIdx.x;
  	if(i<l){ // bad thing -> severely punished 
performance.
    	sdata[tid] = g_idata[i];
    }else{
    	sdata[tid] = 0.0;
...
void d_VectorMult(double *Vec1,double *Total){
  double * d_Vec1;
  double * d_Total;
  double Blocksize=TILE_WIDTH; // Block of 1Dim
...
void Checksum(double *Answer1 , double  *Answer2){
  if(fabs(Answer1[0]-Answer2[0]) < 0.1) cout<<"Nice Work 
Guy"<<endl;
...
Elements to compute. CPU way
  // GPU "Normal" way
  double *Total2 = (double*)malloc((SIZE)*sizeof(double));
  double *Total1 = (double*)malloc(sizeof(double));; // Total 
Variables.
...
"<<T1<<",Seconds"<<endl;
  ...
  cout<<"Parallel Result: "<<Total2[0]<<" At 
"<<T2<<",Seconds"<<endl;
